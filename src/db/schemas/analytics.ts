...
const schools = analyticsSchema.table(
  "schools",
  {
    ...
    pkOffered: boolean("pk_offered").notNull(),
    kgOffered: boolean("kg_offered").notNull(),
    g1Offered: boolean("g1_offered").notNull(),
    g2Offered: boolean("g2_offered").notNull(),
    g3Offered: boolean("g3_offered").notNull(),
    g4Offered: boolean("g4_offered").notNull(),
    g5Offered: boolean("g5_offered").notNull(),
    g6Offered: boolean("g6_offered").notNull(),
    g7Offered: boolean("g7_offered").notNull(),
    g8Offered: boolean("g8_offered").notNull(),
    g9Offered: boolean("g9_offered").notNull(),
    g10Offered: boolean("g10_offered").notNull(),
    g11Offered: boolean("g11_offered").notNull(),
    g12Offered: boolean("g12_offered").notNull(),
...
export const schoolProficiency = analyticsSchema.table(
  ...
  (table) => [
    ...
    check("denominator_non_negative", sql`${table.denominator} >= 0`),
    unique("unique_school_proficiency").on(
      table.ncesSchoolId,
      table.year,
      table.academicSubject,
      table.grade,
...
/**
 * Table: schoolGradeComparison
 * ============================
 *
 * Tracks the change in proficiency between two grades within the same year for a specific school.
 * This table enables queries for cross-grade proficiency changes, such as from 9th to 12th grade.
 *
 * **Frontend Relevance**:
 * - **Analytics Pane**: When a school is selected, this table provides data on how proficiency changes between grades, allowing
 *   users to see trends within the school and compare them to district, state, or national averages.
 */
export const schoolGradeComparison = analyticsSchema.table(
  "school_grade_comparison",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    year: schoolYearEnum("year").notNull(),
    ncesSchoolId: text("nces_school_id")
      ...
      .references(() => schools.ncesSchoolId),
    academicSubject: academicSubjectEnum("academic_subject").notNull(),
    startGrade: gradeEnum("start_grade").notNull(),
    endGrade: gradeEnum("end_grade").notNull(),
    subgroup: subgroupEnum("subgroup").notNull(),
    proficiencyChange: numeric("proficiency_change").notNull(),
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at")
      .defaultNow()
      .$onUpdateFn(() => new Date()),
  ...
  (table) => [
    index("idx_schgradecomp_school").on(table.ncesSchoolId),
    index("idx_schgradecomp_year").on(table.year),
    index("idx_schgradecomp_subj").on(table.academicSubject),
    index("idx_schgradecomp_start_grade").on(table.startGrade),
    index("idx_schgradecomp_end_grade").on(table.endGrade),
    index("idx_schgradecomp_subgroup").on(table.subgroup),
    index("idx_schgradecomp_school_year_subj").on(
      table.ncesSchoolId,
      table.year,
      table.academicSubject,
    ),
    index("idx_schgradecomp_full_filter").on(
      table.ncesSchoolId,
      table.year,
      table.startGrade,
      table.endGrade,
    ...
    check(
      "start_grade_before_end",
      sql`${table.startGrade} < ${table.endGrade}`,
    ),
  ],
)

/**
 * Table: districtGradeComparison
 * ==============================
 *
 * Tracks the change in proficiency between two grades within the same year for a specific district.
 * This table enables queries for cross-grade proficiency changes at the district level.
 *
 * **Frontend Relevance**:
 * - **Analytics Pane**: When a district is selected, this table provides data on how proficiency changes between grades, allowing
 *   users to see trends within the district and compare them to state or national averages.
 */
export const districtGradeComparison = analyticsSchema.table(
  "district_grade_comparison",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    year: schoolYearEnum("year").notNull(),
    ncesDistrictId: text("nces_district_id")
  ...
  (table) => [
    index("idx_distgradecomp_dist").on(table.ncesDistrictId),
    index("idx_distgradecomp_year").on(table.year),
    index("idx_distgradecomp_subj").on(table.academicSubject),
    index("idx_distgradecomp_start_grade").on(table.startGrade),
    index("idx_distgradecomp_end_grade").on(table.endGrade),
    index("idx_distgradecomp_subgroup").on(table.subgroup),
    index("idx_distgradecomp_dist_year_subj").on(
      table.ncesDistrictId,
      table.year,
      table.academicSubject,
    ),
    index("idx_distgradecomp_full_filter").on(
      table.ncesDistrictId,
      table.year,
      table.startGrade,
      table.endGrade,
...
/**
 * Table: stateGradeComparison
 * ===========================
 *
 * Tracks the change in proficiency between two grades within the same year for a specific state.
 * This table enables queries for cross-grade proficiency changes at the state level.
 *
 * **Frontend Relevance**:
 * - **Analytics Pane**: When a state is selected, this table provides data on how proficiency changes between grades, allowing
 *   users to see trends within the state and compare them to national averages.
 */
export const stateGradeComparison = analyticsSchema.table(
  "state_grade_comparison",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    year: schoolYearEnum("year").notNull(),
    state: stateEnum("state")
  ...
  (table) => [
    index("idx_stategradecomp_state").on(table.state),
    index("idx_stategradecomp_year").on(table.year),
    index("idx_stategradecomp_subj").on(table.academicSubject),
    index("idx_stategradecomp_start_grade").on(table.startGrade),
    index("idx_stategradecomp_end_grade").on(table.endGrade),
    index("idx_stategradecomp_subgroup").on(table.subgroup),
    index("idx_stategradecomp_state_year_subj").on(
      table.state,
      table.year,
      table.academicSubject,
    ),
    index("idx_stategradecomp_full_filter").on(
      table.state,
      table.year,
      table.startGrade,
      table.endGrade,
...
/**
 * Table: nationalGradeComparison
 * ==============================
 *
 * Tracks the change in proficiency between two grades within the same year at the national level.
 * This table enables queries for cross-grade proficiency changes nationally, serving as a benchmark for comparison.
 *
 * **Frontend Relevance**:
 * - **Analytics Pane**: Provides national benchmarks for cross-grade proficiency changes, allowing users to compare local trends
 *   (school, district, state) to the broader educational landscape.
 */
export const nationalGradeComparison = analyticsSchema.table(
  "national_grade_comparison",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    year: schoolYearEnum("year").notNull(),
    academicSubject: academicSubjectEnum("academic_subject").notNull(),
  ...
  (table) => [
    index("idx_natgradecomp_year").on(table.year),
    index("idx_natgradecomp_subj").on(table.academicSubject),
    index("idx_natgradecomp_start_grade").on(table.startGrade),
    index("idx_natgradecomp_end_grade").on(table.endGrade),
    index("idx_natgradecomp_subgroup").on(table.subgroup),
    index("idx_natgradecomp_year_subj").on(table.year, table.academicSubject),
    index("idx_natgradecomp_full_filter").on(
      table.year,
      table.startGrade,
      table.endGrade,
      table.academicSubject,
