...
- **Naming**: Use camelCase for variables and functions, ensuring names are descriptive for clarity. Use kebab-case for file names, ensuring names match the functionality or React component.
...
- **React Patterns**:
  ...
  - ALWAYS prefer `undefined` over `null` when using `React.useState` in UI code:
    ...
    ```
- **Error Handling**:
  - Always import the Errors namespace: `import { Errors } from '@/lib/errors'`.
...
- **Nullish Checks**:
  - When checking if a variable is nullish (either `undefined` or `null`), always use the double equal sign with `null`:
    ```typescript
    // CORRECT: Checks if the variable is null or undefined.
    if (someVar == null) {
      // Handle the nullish case
    }

    // INCORRECT: Uses a bang to check for null or undefined.
    if (!someVar) {
      // Handle the nullish case
    }
    ```
